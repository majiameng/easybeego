...
// 根据条件查询单条数据
func (t *Position) Get() error {
	err := orm.NewOrm().QueryTable(new(Position)).Filter("id", t.Id).One(t)
...
// 插入数据
func (t *Position) Insert() (int64, error) {
...
// 更新数据
func (t *Position) Update() (int64, error) {
	o := orm.NewOrm()
	rows, err := o.Update(t)
	if rows == 0 || err != nil {
...
// 删除记录
func (t *Position) Delete() (int64, error) {
	o := orm.NewOrm()
	rows, err := o.Delete(t)
	if rows == 0 || err != nil {
