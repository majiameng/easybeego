...
type Example2 struct {
	...
	Name       string    `orm:"column(name);size(255);null" description:"演示名称"`
...
func init() {
	orm.RegisterModel(new(Example2))
}

// 根据条件查询单条数据
func (t *Example2) Get() error {
	err := orm.NewOrm().QueryTable(new(Example2)).Filter("id", t.Id).One(t)
...
// 插入数据
func (t *Example2) Insert() (int64, error) {
...
// 更新数据
func (t *Example2) Update() (int64, error) {
	o := orm.NewOrm()
	rows, err := o.Update(t)
	if rows == 0 || err != nil {
...
// 删除记录
func (t *Example2) Delete() (int64, error) {
	o := orm.NewOrm()
	rows, err := o.Delete(t)
	if rows == 0 || err != nil {
