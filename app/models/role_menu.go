...
type RoleMenu struct {
	...
	RoleId     int       `orm:"column(role_id);null" description:"配置标题"`
	MenuId     int       `orm:"column(menu_id);null" description:"配置ID"`
...
// 根据条件查询单条数据
func (t *RoleMenu) Get() error {
	err := orm.NewOrm().QueryTable(new(RoleMenu)).Filter("id", t.Id).One(t)
...
// 删除记录
func (t *RoleMenu) Delete() (int64, error) {
	o := orm.NewOrm()
	rows, err := o.Delete(t)
	if rows == 0 || err != nil {
